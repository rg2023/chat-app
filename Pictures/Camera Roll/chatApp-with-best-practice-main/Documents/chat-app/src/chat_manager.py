...
def append_message(room_id, message):
    with open(f'data/rooms/{room_id}.txt', 'a') as room_file:
...
def get_all_messages(room_id):
    with open(f'data/rooms/{room_id}.txt', 'r') as room_file:
...
def clear_messages(room):
  ...
  if request.method == "POST":
    ...
    with open(f'data/rooms/{room}.txt', 'r') as f:
        lines=f.readlines()  
    with open(f'data/rooms/{room}.txt', 'w') as file:
            for line in lines:
